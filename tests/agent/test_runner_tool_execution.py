...
@pytest.mark.asyncio
async def test_runner_blocks_repeated_image_reads():
    provider = MagicMock()
    captured_final_call: list[dict] = []
    call_count = {"n": 0}
            ...
            return LLMResponse(
                ...
                tool_calls=[
                    ToolCallRequest(
                        id=f"call_{call_count['n']}",
                        name="read_file",
                        arguments={"path": "/tmp/receipt.jpg"},
                    )
                ],
                usage={},
            )
    ...
    tools.execute = AsyncMock(return_value=[
        {
            ...
            "_meta": {"path": "/tmp/receipt.jpg"},
        },
        {"type": "text", "text": "(Image file: /tmp/receipt.jpg)"},
    ])

    result = await AgentRunner().run(make_run_spec(provider,
        initial_messages=[{"role": "user", "content": "what does this receipt say?"}],
        tools=tools,
        model="test-model",
        max_iterations=4,
        max_tool_result_chars=_MAX_TOOL_RESULT_CHARS,
    ...
    assert "repeated image read blocked" in blocked_tool_message["content"]
